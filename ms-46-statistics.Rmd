...
include_graphics('assets/figures/twobytwo.jpg')
```
...
include_graphics('assets/figures/fixed-random.jpg')
```
...
16명의 C~max~와 AUC~last~가 나온 표입니다. 먼저 그림을 살펴보겠습니다.
...
figA <- ggplot(sad_indi_pk_log, aes(x=Dose, y=Cmax)) +
  ...
  labs(x = 'Dose (mg)', y = expression('C'[max]*' (ng/mL) / Dose (mg)'),#'Cmax / Dose', 
...
figB <- ggplot(sad_indi_pk_log, aes(x=Dose, y=AUClast)) +
  ...
  theme_bw() +
  scale_x_continuous(breaks = c(50, 100, 200, 400)) +
  labs(x = 'Dose (mg)', y = expression('AUC'[(0-last)]*' (ng·hr/mL) / Dose (mg)'),#'Cmax / Dose', 
