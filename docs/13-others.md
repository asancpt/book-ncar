...
```
## Error in my.results.automatic$result %>% filter(grepl(pattern = "cmax|aucinf", : 함수 "%>%"를 찾을 수 없습니다
```
...
```
## Error in Theoph %>% rename(ID = Subject, TIME = Time, DV = conc) %>% write_csv("Theoph_ncappc.csv"): 함수 "%>%"를 찾을 수 없습니다
```
...
<img src="13-others_files/figure-html/unnamed-chunk-4-1.png" width="672" /><img src="13-others_files/figure-html/unnamed-chunk-4-2.png" width="672" />
